...
- **Additional architecture observations preserved as comments:** `workers.unlocked_achievements` duplicates data tracked in `worker_achievements`; `campaigns` has two independent admin-detection paths.
- **Phase 6.2 security fixes (resolved, no longer open issues):** `workers_update`, `bookings_worker_read`, `bookings_update`, and `worker_achievements_insert` previously granted access with no ownership restriction — all four now scope access to `auth.uid()` matching the relevant row. The `worker-documents` bucket was private at the bucket level but its storage policies granted public read/write scoped only by `bucket_id` — policies now additionally require the caller's `auth.uid()` to match a `workers.document_name` owned by them (or `is_admin()`), and INSERT/UPDATE/SELECT are all scoped to `authenticated` rather than `public`. A critical self-role-escalation vulnerability in `users_own` (any user could set their own `role` to `'admin'`) was also found and fixed via a `SECURITY DEFINER` trigger (`prevent_role_self_escalation`), since a naive RLS `WITH CHECK` fix caused infinite recursion. All findings were confirmed exploitable live, then fixed and re-verified live, during the Phase 6.2 audit.
