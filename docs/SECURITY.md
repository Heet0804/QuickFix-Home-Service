...
- **Phase 8 finding, now resolved for one specific write path.** During Phase 8 development, an admin-issued ban write against `workers` (`UPDATE`) was found to be **silently blocked** by the pre-existing `workers_update` RLS policy, which only permitted a worker to update their own row (`auth.uid() = id`) — there was no admin-scoped UPDATE policy on `workers` at all prior to this phase. Because Supabase's REST layer returns no error when an RLS policy filters a write down to zero matched rows, `admin.js`'s ban-confirmation code reported success on every attempt regardless of whether the database was actually updated. This was fixed by adding an admin-scoped UPDATE policy (`admins_can_update_any_worker`) and by changing the client to append `.select()` to the write and treat an empty result as a failure. This is presented as a positive finding — a real authorization gap was caught and closed — but it also demonstrates concretely that **the class of gap described in this section is not hypothetical**: an admin action failed in exactly the way this document warned it could, and would have gone unnoticed indefinitely without this direct verification.
...
Government ID documents (Aadhaar/PAN) and profile photos are uploaded to two Supabase Storage buckets (`worker-documents`, `worker-photos`) at worker signup, with randomized filenames (`worker_<timestamp>_<random>.<ext>`). `worker-photos` is a **public** bucket at the Supabase project level and is retrievable by anyone with the URL, with no authentication required.

**Correction (Phase 8): `worker-documents` public-URL retrieval does not actually work.** Prior revisions of this document (following `SRS.md`'s stated assumption) described `worker-documents` as accepting public-URL retrieval identically to `worker-photos`. This was verified false during Phase 8's admin-portal work: `worker-documents` is a **private** bucket at the project level, and Supabase's `/object/public` retrieval endpoint only serves objects from buckets flagged public at the bucket level — this is independent of, and not overridden by, any `storage.objects` RLS policy granting SELECT on that bucket's objects. The practical effect observed was that the stored `document_url` (a public-URL string generated by `auth.js` at upload time) never resolved for any caller, admin included — it rendered as a broken image, not a security exposure. The admin portal now retrieves these documents via a short-lived (5-minute) signed URL (`createSignedUrl`), gated by a new `admins_can_select_worker_documents` policy on `storage.objects`. **Net effect on this document's risk assessment:** ID documents are less exposed than previously stated — they are not retrievable by an arbitrary party who obtains the stored URL string, only by a party who can pass the bucket's RLS and generate their own signed URL. No encryption-at-rest, retention period, or deletion policy is defined anywhere in the inspected source (`PRD.md` §22A.5, an explicitly flagged open compliance item that the PRD itself states must be resolved before production launch) — this part of the finding is unchanged.
...
## 10A. Phase 8 Security Additions

- **Worker ban enforcement.** `workers.banned_until` gates new logins (`auth.js`) and forces a sign-out on an active session via a Supabase Realtime channel scoped to the worker's own row, with a poll-based fallback. Both checks compare a plaintext timestamp column read by the client; there is no independent RLS/trigger-level enforcement confirmed to block a banned worker's own writes at the database layer beyond ordinary row-ownership policies.
- **Admin-write RLS gap found and closed.** See Section 3.4 — a genuine authorization gap (no admin-scoped UPDATE policy on `workers`) was found and fixed within this phase, not merely hypothesized.
- **Server-authoritative streak/bonus.** `handle_review_streak()`, a Postgres trigger fired `AFTER INSERT ON reviews`, computes and writes `workers.positive_streak`/`bonus_balance` entirely server-side — the one Phase 8 feature with no client-side write path at all, and therefore not subject to the "client-side business logic" gap described throughout Section 3–4 of this document for any other feature.
- **Review data exposure.** `reviews_read`'s RLS policy grants unrestricted public SELECT (`USING: true`), including `comment` and the new `tags` column. The product's intent — full review detail visible to admins only, workers restricted to aggregated positive-tag counts via `get_worker_positive_tags()` — is enforced only by client-side convention (the worker dashboard simply never queries `reviews` directly); a client bypassing the UI and querying `reviews` directly via the Supabase REST endpoint would not be blocked by RLS from reading any customer's rating, comment, or negative tags about any worker.
- **Inconsistent self-access on new history tables.** `worker_bonuses` grants a worker SELECT on their own rows (`workers_can_select_own_bonuses`); `worker_bans` grants no equivalent self-access — a worker cannot see their own ban history under the current policy set, an inconsistency not reconciled anywhere in the schema or client code.
