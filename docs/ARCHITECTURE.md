...
- **Worker ban management (Phase 8).** `openBanModal()`/`confirmBanWorker()` present a duration picker (amount + unit: minutes/hours/days/weeks) seeded with a suggested value that escalates against the worker's `ban_count` (1st offense: 5 hours, 2nd: 1 day, 3rd+: 5 days), always overridable. The write itself goes through `sb.from('workers').update({...}).select()` — the `.select()` is load-bearing: it is how the client detects an RLS-blocked write (Section 10.1 below) instead of reporting a false success. A parallel, permanent audit row is written to a new `worker_bans` table on every ban, independent of `workers.banned_until` (which reflects only the current/latest ban).
- **Ban visibility gating.** The Reviews tab's Ban action is computed per-row from three conditions, all evaluated client-side against already-loaded data: the review's `tags` array contains at least one negative tag, the review's `rating` is below 4, and the worker is not already currently banned (`banned_until > now`). Any one of these failing hides the action entirely, showing a static "Banned" badge or `—` instead.
- **Worker verification.** `setWorkerVerification()` writes `workers.verification_status` (`'pending'|'approved'|'rejected'`, default `'pending'`); the Workers tab renders Approve/Reject as pill-shaped buttons only while `status !== 'approved'`, replacing them with a static badge once approved, to avoid a duplicate "approved" indicator alongside the Verification column's own status badge.
- **Signed-URL document access.** Because `worker-documents` is a private Storage bucket, `openAdminDocView()` calls `sb.storage.from('worker-documents').createSignedUrl(documentName, 300)` on demand rather than using the stored `document_url` public-URL string, which cannot resolve against a private bucket regardless of the bucket's `storage.objects` RLS policies (Section 14, Security Architecture, updated below).
- **Realtime (Phase 8) — correction to Section 11.** `admin.js` is no longer realtime-silent. Three channels (`admin-reviews`, `admin-users`, `admin-workers`) keep the Reviews, Users, Workers, and Banned Workers tabs current without a manual refresh; see Section 11 for the corrected realtime-architecture description.

---

## 11. Tracking Architecture

**Correction, Phase 8.** This section's original framing implied a single global Realtime subscription existed in the entire codebase (the worker booking-list channel described below). As of Phase 8, `dashboard.js` also carries a second channel (`worker-selfrow-<id>`, scoped to the worker's own `workers` row, used to force-logout a worker the instant an admin bans them — see Section 10), and `admin.js` carries three more (Section 10). Tracking itself remains poll/observer-driven on both sides, unaffected by any of these additions; this section's original tracking-specific content is otherwise unchanged.
...
| Worker ban enforcement (Phase 8) | A worker's `banned_until` gates both new logins (`auth.js`'s `doLogin()` blocks sign-in if `banned_until` is in the future) and an already-active session (`dashboard.js`'s realtime self-row channel + poll fallback force a client-side sign-out) | Both checks are client-issued reads of a plaintext timestamp column; there is no server-side (RLS/trigger) enforcement confirmed to independently block a banned worker's writes at the database layer, beyond whatever `workers_update`/`admins_can_update_any_worker` RLS already restricts by row ownership |
| Positive-streak/bonus crediting (Phase 8) | Computed and written entirely by a Postgres trigger (`handle_review_streak()`, `AFTER INSERT ON reviews`), not by client code | This is the one Phase 8 addition that is server-authoritative by construction — the client never writes `workers.positive_streak`/`bonus_balance` directly, closing the class of gap described elsewhere in this table for this specific feature |
